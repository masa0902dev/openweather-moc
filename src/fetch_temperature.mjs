import { readAllLineAnyOneColCSV, appendCSV, target_urls_all } from "../modules/module.js";
...
async function fetchTemperature(city, format_date) {
  ...
  try {
    ...
    const avg_temp = (res.temperature.min + res.temperature.max) / 2;
...
function formatDateForCSV(now) {
  return `${now.getFullYear()}/${now.getMonth() + 1}/${now.getDate()}`;
}

async function saveTemperature(now, year, city, date_csv_line, temp) {
...
function getNormCount(i) {
  let norm_count = String(i + 1);
